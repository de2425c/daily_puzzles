...
async function createRiverSim() {
  ...
  try {
    const result = await api.createRiverSim(
      sim.value.id,
      currentPath.value,
  ...
  <div class="river-builder">
    ...
    <template v-else-if="sim">
      ...
      <div v-if="isTerminal && rangesInfo" class="card terminal">
        ...
        <!-- River Card Input -->
        <div class="river-input">
          <label>River Card (optional, leave blank for random)</label>
          <input
            v-model="riverCard"
            placeholder="e.g., 8h"
            maxlength="2"
            class="river-card-input"
          />
        </div>
      ...
      <div class="bottom-actions">
        <button
          ...
          @click="createRiverSim"
          :disabled="creating"
        >
          {{ creating ? 'Creating River Sim...' : 'Create River Sim (~$2)' }}
        </button>
